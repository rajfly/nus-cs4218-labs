...
public class SortApplication implements SortInterface {
    ...
    public void run(String[] args, InputStream stdin, OutputStream stdout) throws SortException {
        ...
        try {
            ...
            throw (SortException) new SortException(e.getMessage()).initCause(e);
        }
        StringBuilder output = new StringBuilder();
        try {
            if (sortArgsParser.getFileNames() == null || sortArgsParser.getFileNames().isEmpty() || (sortArgsParser.getFileNames()).get(0).isEmpty()) {
    ...
    public String sortFromFiles(Boolean isFirstWordNumber, Boolean isReverseOrder, Boolean isCaseIndependent,
        ...
        for (String file : fileNames) {
            ...
            try {
                ...
                throw (SortException) new SortException(ERR_IO_EXCEPTION).initCause(e);
            }
            try {
            ...
            public int compare(String str1, String str2) {
                ...
                if (isFirstWordNumber && !temp1.isEmpty() && !temp2.isEmpty()) {
                    ...
                    if (Character.isDigit(chunk1.charAt(0)) && Character.isDigit(chunk2.charAt(0)) ||
                            (chunk1.length() > 1 && Character.isDigit(chunk1.charAt(1))) ||
                            (chunk2.length() > 1 && Character.isDigit(chunk2.charAt(1)))) {
                        // Handle negative numbers
                        if (chunk1.startsWith("-") && chunk2.startsWith("-")) {
                            result = new BigInteger(chunk2.substring(1)).compareTo(new BigInteger(chunk1.substring(1)));
                        } else if (chunk1.startsWith("-")) {
                            // chunk1 is negative, so it comes before chunk2
                            result = -1;
                        } else if (chunk2.startsWith("-")) {
                            // chunk2 is negative, so it comes before chunk1
                            result = 1;
                        } else {
    ...
    private String getChunk(String str) {
        ...
        chunk.append(chr);
        final boolean extractDigit = Character.isDigit(chr) || (chr == '-' && startIndexLocal < strLen && Character.isDigit(str.charAt(startIndexLocal)));
